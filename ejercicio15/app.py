...
class App:
    ...
    def ejecutar(self):
        ...
        archivo_subido = st.file_uploader("Selecciona tu archivo", type=["nmap"])

        if archivo_subido is None:
            st.info("Primero sube un archivo .nmap")
            return

        archivo = Archivo(archivo_subido)

        if not archivo.es_nmap():
            st.error("El archivo debe ser .nmap")
            return
